const {join} = require('path')
const roadmap = require(join(__dirname, '..', '..', 'lib', 'roadmap'))

const app = {
  'name': 'PowerApps',
  'description': 'Codeless App Development',
  'icon': '/assets/icons/powerapps.png',
  'video': 'http://www.youtube.com/embed/UCpmP6FOK-s',
  'rating': '69%',
  'color': '5c005c',
  'testimonials': [
    {
      'text': '"PowerApps is a quick way to easily create applications that will allow users to consume and edit data from multiple different sources."'
    },
    {
      'text': '"Easy to use and create apps; Visual editor on the web and as an app; Coding is very easy and simple like excel"'
    }
  ],
  'features': [
    {
      'name': 'Codeless Development',
      'description': 'Quickly create apps without coding to keep data at users fingertips without the complexity.'
    },
    {
      'name': 'Fast',
      'description': 'Deploy apps to your organization in hours not weeks with simple drag and drop development.'
    },
    {
      'name': 'Integrations',
      'description': 'Integrate a PowerApps frontend with data from SharePoint, OneDrive, Excel, Salesforce and more.'
    }
  ],
  'licenses': [
    'Office 365 Business Premium',
    'Office 365 Business Essentials',
    'Office 365 Enterprise E1',
    'Office 365 Enterprise E3',
    'Office 365 Enterprise E5',
    'Office 365 A1',
    'Office 365 A3',
    'Office 365 A5'
  ],
  'platforms': {
    'web': 'https://web.powerapps.com',
    'pc': 'https://powerapps.microsoft.com/en-us/downloads/',
    'android': 'https://play.google.com/store/apps/details?id=com.microsoft.msapps&hl=en_US',
    'iPhone': 'https://itunes.apple.com/us/app/powerapps/id1047318566?mt=8'
  },
  'resources': [
    {
      'title': 'Homepage',
      'url': 'https://powerapps.microsoft.com/en-us/'
    },
    {
      'title': 'Twitter',
      'url': 'https://twitter.com/hashtag/PowerApps'
    },
    {
      'title': 'Roadmap',
      'url': 'https://products.office.com/en-us/business/office-365-roadmap?filters=%26freeformsearch=powerapps'
    },
    {
      'title': 'Training',
      'url': 'https://docs.microsoft.com/en-us/powerapps/guided-learning/'
    },
    {
      'title': 'Documentation',
      'url': 'https://powerapps.microsoft.com/en-us/search/documentation/'
    },
    {
      'title': 'YouTube',
      'url': 'https://www.youtube.com/channel/UCGfWR2ekfRFckLjev6eQYLg'
    },
    {
      'title': 'UserVoice',
      'url': 'https://powerusers.microsoft.com/t5/PowerApps-Ideas/idb-p/PowerAppsIdeas/tab/most-kudoed'
    }
  ],
  'competitors': [
    {
      'name': 'Google App Maker',
      'url': 'https://developers.google.com/appmaker/',
      'image': '/assets/icons/google-app-maker.png'
    },
    {
      'name': 'Mozilla Appmaker',
      'url': 'https://wiki.mozilla.org/Appmaker',
      'image': '/assets/icons/mozilla-appmaker.png'
    }
  ],
  'requests': [
    {
      'votes': 600,
      'title': 'Share with external users'
    },
    {
      'votes': 500,
      'title': 'Attach a file to form'
    },
    {
      'votes': 460,
      'title': 'Printing'
    },
    {
      'votes': 440,
      'title': 'PowerApps must also see SQL Azure Queries, not only Tables'
    },
    {
      'votes': 400,
      'title': 'Save attachments to SharePoint list item'
    }
  ],
  'title': 'PowerApps',
  'datePublished': '2018-06-07T06:00:00.000Z',
  'dateModified': '2018-06-07T06:00:00.000Z',
  'image1200x1200': '/apps/assets/powerapps-1200x1200.jpg',
  'image1200x900': '/apps/assets/powerapps-1200x900.jpg',
  'image1200x675': '/apps/assets/powerapps-1200x675.jpg',
  'image400x300': '/apps/assets/powerapps-400x300.jpg',
  'canonical': '/apps/powerapps',
  'template': '/apps/app.template.pug'
}
app.updates = roadmap.getLatest(app.tag)
module.exports = app